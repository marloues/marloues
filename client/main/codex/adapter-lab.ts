...
export function readAdapterLabReport(limit = 12): AdapterLabReport {
  const rawSessions = store
    .getSessions()
    .filter((session) =>
      session.messages.some((message) => message.rawEvents?.length),
    )
  ...
  const sessions = rawSessions.map((session) => {
    ...
    for (const turn of rawTurns) {
      for (const [method, count] of Object.entries(turn.methods)) {
        if (!knownRawMethod(method))
          unknownRawTypes[method] = (unknownRawTypes[method] ?? 0) + count;
      }
    }

    const runtimeThreadId = session.runtimeThreadId;
    ...
    if (matchedLog) {
      for (const turn of matchedLog.turns) {
        for (const [key, count] of Object.entries(turn.itemCounts)) {
          if (!knownJsonlKeys.has(key))
            unknownJsonlTypes[key] = (unknownJsonlTypes[key] ?? 0) + count;
        }
      }
    }

    const matchedTurnIds = new Set(
      matchedLog?.turns.map((turn) => turn.turnId).filter(Boolean) ?? [],
    );
    const rawToolCount = sum(rawTurns.map((turn) => turn.toolCount));
    const logToolCount = sum(
      matchedLog?.turns.map((turn) => turn.toolCount) ?? [],
    );
    const logTurnsById = new Map(
      (matchedLog?.turns ?? []).map((turn) => [turn.turnId, turn]),
    );
    const rawTurnsWithParity = rawTurns.map((turn) => ({
      ...turn,
      parity: compareTurnParity(turn, logTurnsById.get(turn.turnId)),
    }));
    const mismatchedTurns = rawTurnsWithParity.filter(
      (turn) => !turn.parity.matches,
    ).length;
    ...
    return {
      ...
      coverage: {
        ...
        completedRawTurns: rawTurns.filter(
          (turn) => turn.hasCompleted || turn.status === "completed",
        ).length,
        matchedLogTurns: rawTurns.filter((turn) =>
          matchedTurnIds.has(turn.turnId),
        ).length,
        rawToolCount,
        logToolCount,
...
function reduceRawTurns(
  session: ReturnType<typeof store.getSessions>[number],
): AdapterLabRawTurn[] {
  ...
  session.messages.forEach((message, index) => {
    ...
    for (const event of message.rawEvents) {
      ...
      if (rawType && !knownRawItemTypes.has(rawType))
        methods[`item:${rawType}`] = (methods[`item:${rawType}`] ?? 0) + 1;
    }

    const toolCount = normalizedTurn.items.filter(
      (item) =>
    ...
    const text = stripThinkTags(
      normalizedTurn.finalText || (message.content ?? ""),
    );

    turns.push({
      messageIndex: index,
      user: previousUser(session.messages, index),
      status: normalizedTurn.hasFailed
        ? "failed"
      ...
      toolCount,
      toolNames: unique(
        normalizedTurn.items.filter(isComparableToolItem).map(rawToolName),
      ),
...
function findMatchingLog(
  ...
  rawTurns: AdapterLabRawTurn[],
  runtimeThreadId?: string,
): AdapterLabLogFile | null {
  const threadIds = new Set(
    [runtimeThreadId, ...rawTurns.map((turn) => turn.threadId)].filter(Boolean),
  );
  ...
  for (const path of files) {
    ...
    const sessionId = stringValue(
      events.find((event) => event.type === "session_meta")?.payload?.id,
    );
  ...
  for (const event of events) {
    ...
    if (
      ...
      const callId =
        ...
        `${current.turnId}-${calls.size}`;
      calls.add(callId);
      current.toolCount = calls.size;
      const toolName =
        stringValue(payload.name) || stringValue(payload.type) || "tool_call";
    ...
    if (event.type === "event_msg" && payload.type === "task_complete") {
      ...
      current.text = stripThinkTags(
        stringValue(payload.last_agent_message) || current.text,
      );
      current.preview = preview(current.text);
...
function buildReferenceCases(
  ...
  return referenceCases.map((testCase) => {
    const desktopCandidates = candidates
      .filter(
        (match) =>
          match.source === "desktop" &&
          hasRequiredFeatures(match.features, testCase.requiredFeatures),
      )
      .map((match) =>
        scoreReferenceMatch(
          match,
          testCase.requiredFeatures,
          testCase.optionalFeatures ?? [],
        ),
      )
...
function scoreReferenceMatch(
  ...
  const requiredScore =
    required.filter((feature) => match.features.includes(feature)).length * 10;
  const optionalScore =
    optional.filter((feature) => match.features.includes(feature)).length * 3;
  const sourceScore =
    match.source === "desktop" ? 5 : match.source === "codex-web" ? 2 : 0;
...
function compareTurnParity(
  rawTurn: AdapterLabRawTurn,
  logTurn: AdapterLabLogTurn | undefined,
): AdapterLabTurnParity {
...
function rawToolName(item: NormalizedThreadItem): string {
  if (item.type === "command_execution")
    return item.command || item.rawType || "shell_command";
...
function featuresForRawTurn(turn: AdapterLabRawTurn): string[] {
  ...
  if (turn.hasCompleted || turn.status === "completed")
  ...
  for (const method of Object.keys(turn.methods)) {
    ...
    if (method.includes("customTool") || method.includes("custom_tool"))
      features.add("custom_tool");
    if (method.includes("localShell") || method.includes("commandExecution"))
      features.add("shell_command");
    if (method.includes("fileChange") || method.includes("patch"))
      features.add("file_patch");
  }
...
function featuresForLogTurn(turn: AdapterLabLogTurn): string[] {
  ...
  if (turn.toolCount > 0) features.add("tool_call");
  if (
    turn.itemCounts["response_item:function_call_output"] ||
    turn.itemCounts["response_item:custom_tool_call_output"]
  ) {
  ...
  if (
    turn.itemCounts["event_msg:patch_apply_end"] ||
    turn.toolNames.includes("apply_patch")
  )
    features.add("file_patch");
  if (
    turn.itemCounts["response_item:custom_tool_call"] ||
    turn.itemCounts["response_item:custom_tool_call_output"]
  )
    features.add("custom_tool");
  if (
    turn.itemCounts["event_msg:mcp_tool_call_end"] ||
    turn.toolNames.some(
      (name) => name.includes("browser") || name.includes("mcp"),
    )
  )
    features.add("mcp_tool");
  if (turn.status === "failed" || /Exit code:\s*[1-9]/.test(turn.preview))
...
): AdapterLabReferenceMatch["source"] {
