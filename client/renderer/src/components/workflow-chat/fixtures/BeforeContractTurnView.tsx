// Frozen review baseline edebdae. Used only by the development comparison page.
import { memo } from "react";
import type { WorkflowMessageBlock as WorkflowMessageBlock } from "../../../../../shared/adapters/workflow-messages-to-read-thread";
import { AssistantTurnHeader } from "../turns/AssistantTurnHeader";
import { WorkflowTurnFooterView } from "../turns/TurnFooterView";
...
export const BeforeContractTurnView = memo(function BeforeContractTurnView(
  props: Props,
) {
  const {
    message,
  ...
  const label = workflowTurnStatusLabel(message, {
    hasActivityItems,
    isLastStreaming,
  });
      ...
      >
        ...
        {!message.continuesPreviousTurn ? (
          <AssistantTurnHeader
            ...
            duration={
              durationMs != null
                ? workflowTurnDurationLabel(durationMs, { running })
                : null
            }
            expanded={expanded}
        ...
        {expanded ? (
          ...
          <div
            ...
            data-kind="message-turn-summary"
          >
        ...
        {/* 流式状态：只显示在真正活跃的（最后一个）turn 上；引导的前置段保持
            running 但不再显示"正在思考"。 */}
        {isLastStreaming && expanded ? (
          <MessageStatusRow startedAt={message.startedAt} />
        ) : null}
